...
<style type="text/css">
	...
	.bienFondo{
		background: rgba(142,244,150,1);
	}
	.malFondo{
		background: rgba(239,104,97,1);
	}
	#miContenido>div:not(#head){
...
<script type="text/javascript" src="scripts/cedulasMalas.js"></script>
...
<div id="miContenido">
	...
	<div>
		<p>
			Egresados Bienvenidos.</br>
			Para nosotros la Oficina de Proyección Institucional,
			es de mucha importancia que Usted
			como egresado de nuestras institución 
			suministre algunos datos de importancia 
			para nuestra oficina y para la I.U. CESMAG,
			esto con el fin de realizar un seguimiento apropiado 
			de todos nuestros egresados.</br>
	...
	<div>
		<form id="fEgresado">
			...
			<input id="egresado" type="text" placeholder="Escribe tu Cedula"/>
			<label for="passE">Contraseña</label>
			<input id="passE" type="password" />	
			<span></span>
			<a id="problema" href="#" style="text-align:left">Mi cedula no funciona</a>
			<span></span>
			<input type="submit" id="btEgresado" value="Entrar">
		</form>
		<form id="fAdmin">
			<label for="admin">Administrador</label>
			<input id="admin" type="text" placeholder="Nombre de usuario"/>
			<label for="pass">Contraseña</label>
			<input id="pass" type="password" /><span></span>
			<input type="submit" id="btAdm" value="Entrar">
		</form>
	</div>
	<div id="cedulasMalas">
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$('#problema').click(function(event) {
				//cedulasMalas();
				alert("Apreciado egresado:\n\nSi Usted tiene inconvenientes para acceder al sistema de actualización de datos, favor comuniquese al correo:\n\negresados@iucesmag.edu.co\n\nPara solucionar sus inquietudes.\n\nCordialmente,\n\nOficina de Proyección Institucional\nTelefono fijo: 7216535 extensión 333..");
			});
			$('#fAdmin').submit(function(event) {
				event.preventDefault();
				var id = $('#admin').val();
				var pass = $('#pass').val();
				$.post("includes/sqLite.php",{op:"loginAdm", id:id,pass:pass},function(res){
					if (res == "true") {
						$(location).attr('href','admin.php');
					}else{
						mensaje("USUARIO O CONTRASEÑA INVALIDOS","mal")
					}
				})
			});
			
			$('#fEgresado').submit(function(event) {
				event.preventDefault();
				var id = $('#egresado').val();
				var pass = $('#passE').val();
				$.post("includes/procesos.php",{op:"loginEgresado", id:id, pass:pass},function(res){
					if (res == "true") {
						$(location).attr('href','index.php');
					}else{
						mensaje("Identificacion invalida","mal")
					}
				})
			});
		});
