...
<style type="text/css">
	...
	#progressbar{
		...
		width:100%;
		height:30px;
		background:url(css/Cesmag/images/animated-overlay2.gif) rgba(0,51,204,0.5);		
		text-align:center;
		font-weight:bold;
		font-size:15px;
	}
</style>
<link rel="stylesheet" href="css/Cesmag/jquery-ui.css" type="text/css" />
<link rel="stylesheet" href="css/flexigrid.css" type="text/css" />
<script type="text/javascript" src="scripts/jquery.js"></script>
<script type="text/javascript" src="scripts/jquery-ui.js"></script>
<script type="text/javascript" src="scripts/tablePagination.js"></script>
<script type="text/javascript" src="scripts/flexigrid.js"></script>
<script type="text/javascript">
	$(document).ready(function(e) {	
		$("th").resizable({alsoResize: "td",resize: function(event, ui) { ui.size.height = ui.originalSize.height;},handles:'e'});			
		$("#progressbar").hide();
		$("table").show();
...
$_SESSION['numero'] = 0;
	...
	//consulta sql
	$sql ="select  
distinct per.id as identificacion,tipi.num_doc as num_identi,ciu.nombre as ciudexp,per.pri_ape,per.seg_ape,
per.pri_nom,per.seg_nom,per.genero,estp.fecha_egre as fecha_grado, progs.nombre as programa,fac.nombre as facultad,
tit.nombre as titulo,
(select descripcion from ubicapersonas as ub where ubip.id_persona=ub.id_persona and id_ubicacion=1) as direccion,
(select descripcion from ubicapersonas as ub where ubip.id_persona=ub.id_persona and id_ubicacion=2)
as telefono,
(select descripcion from ubicapersonas as ub where ubip.id_persona=ub.id_persona and id_ubicacion=3)
as celular,
(select descripcion from ubicapersonas as ub where ubip.id_persona=ub.id_persona and id_ubicacion=4)
as correo , estc.descripcion as estado_civil

from
personas as per join tiposidentificaciones as tipi on per.id = tipi.id_persona join ciudades as ciu on
tipi.id_ciudexp = ciu.id join estudiospers as estp on per.id = estp.id_persona join 
titulos as tit on estp.id_titulo = tit.id join programas as progs on
progs.id = tit.id_programa join facultades as fac on fac.id = progs.id_facultad join 
ubicapersonas as ubip on ubip.id_persona = per.id join ubicaciones as ubi on
ubip.id_ubicacion=ubi.id join estadosciviles as estc on per.id_estadocivil = 
estc.id
";
	...
	$_SESSION["tDatos"] = pg_num_rows($result);
	$_SESSION["tPaginas"] = $_SESSION["tDatos"] / 20;
	
	
	//recorrer con while resultados obtenidos
	while($row=pg_fetch_array($result))
		{
		echo "<tr>
				<td>".$row['identificacion']."</td>
				<td>".$row['num_identi']."</td>
				<td>".$row['ciudexp']."</td>
				<td>".$row['pri_nom']."</td>
				<td>".$row['seg_nom']."</td>
				<td>".$row['pri_ape']."</td>
				...
				<td><a href='eliminaregresado.php?id=".$row['identificacion']."&
												  tipo_iden=".$row['identificacion']."&
												  num_iden=".$row['num_identi']."&
												  ciud_exp=".$row['ciudexp']."&
												  pri_nom=".$row['pri_nom']."&
												  seg_nom=".$row['seg_nom']."&
												  pri_ape=".$row['pri_ape']."&
												  seg_ape=".$row['seg_ape']."&	
				...
				'>Eliminar</a></td>
				<td><a href='formularioactualizaregresados.php?id=".$row['identificacion']."&
												  tipo_iden=".$row['num_identi']."&
												  num_iden=".$row['num_identi']."&
												  ciud_exp=".$row['ciudexp']."&
												  pri_nom=".$row['pri_nom']."&
												  seg_nom=".$row['seg_nom']."&
...
?>
	...
	<input type="button" id="pr" value="anterior">
    <input type="button" id="bt1" value="1">
    <input type="button" id="bt2" value="2">
    <input type="button" id="bt3" value="3">
    <input type="button" id="bt4" value="4">
    <input type="button" id="sg" value="siguiente">
